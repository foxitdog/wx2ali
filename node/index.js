...
rl
    .on('line', function (line, lineCount, byteCount) {
        ...
        if (state === "") {
            ...
            // if (config.methods.push(aTob[0]) != config.toMethods.push(aTob[1])) {
            //     throw new Error("配置中的参数填写错误，请检查");
            // }
        } else if ("JS" === state) {
            var aTob = line.split("--->");
            addJSRegexp(aTob[0], aTob[1]);
            // if (config.JSRegexp.push(aTob[0]) != config.JSToRegexp.push(aTob[1])) {
            //     throw new Error("配置中的参数填写错误，请检查");
            // }
        } else if ("AXML" === state) {
            var aTob = line.split("--->");
            addAXMLRegexp(aTob[0], aTob[1]);
            // if (config.AXMLRegexp.push(aTob[0]) != config.AXMLToRegexp.push(aTob[1])) {
            //     throw new Error("配置中的参数填写错误，请检查");
            // }
        } else if ("DIR" === state) {
            config.dir = line;
            console.log("修改文件的路径切换到：" + line);
        } else if("OVER"===line){
            main();
        }
    })
...
function main(){
    ...
    clearSuffix();
    //	修改的文件后缀名
    addUpdateSuffix("axml");//这边的order是WX2ANT 所以是修改该后缀名的文件
    addUpdateSuffix("js");
    setOrder(WX2ANT);
    HandleFile(config.dir);
}
